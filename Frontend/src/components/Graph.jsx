...
const Graph = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const response = await getIncomebyUid(userId)
        ...
        expensesData.forEach((item) => {
            const { incCategory , incAmount } = item;
            if (counts[incCategory]) {
                counts[incCategory]+=incAmount;
            } else {
                counts[incCategory] = incAmount;
            }
        });
  ...
  return (
    ...
    <div className='chart-container'/>
        <h2 style={{color:"darkcyan",letterSpacing:"2px"}}>&nbsp;&nbsp;Analysis of your incomes</h2><br/>
      <div>
